...
while true; do
  ...
  echo "Please unplug the module and plug it back to reset it"
  echo "Led on module should now be **RED** after replug"
  read -p "Once OK, press enter to continue" DUMMY
  waitDevice
  ...
  if [ "$retval" == 0 ] ; then
    ...
    if [ "$retval" == 0 ] ; then
      ...
      # usb_modeswitch -v 0x0403 -p 0x6015 --reset-usb --quiet
      echo "Please unplug the module and plug it back to reset it"
      echo "Led on module should now be **GREEN** after replug"
      read -p "Once OK, press enter to continue" DUMMY
      waitDevice
