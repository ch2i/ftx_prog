...
while true; do
  ...
  echo "-------------------------------------------"
	...
	echo "-------------------------------------------"
	#command="./ftx_prog --new-serial-number TINFO-$NSERNUM --cbus 1 PWREN --cbus 2 TxLED --invert rxd"
	safeRunCommand "./ftx_prog --restore teleinfo.eep"
	retval=$?
	...
	if [ "$retval" == 0 ] ; then
		...
		if [ "$retval" == 0 ] ; then
			...
			echo "-------------------------------------------"
			safeRunCommand "./ftx_prog --new-serial-number TINFO-$SERNUM --cbus 1 PWREN --cbus 2 TxLED --invert rxd"
			retval=$?
